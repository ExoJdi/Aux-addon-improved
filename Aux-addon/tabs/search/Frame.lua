...
for _ = 1, 5 do
    ...
    table:SetHandler('OnClick', function(row, button)
        if (IsShiftKeyDown() or IsAltKeyDown()) and current_search.table:GetSelection().record == row.record then
            if button == 'LeftButton' then
                
                if IsShiftKeyDown() and not IsAltKeyDown() and not (ChatFrame1EditBox and ChatFrame1EditBox:IsVisible()) then
                    prompt_buyout_quantity(row.record)
                else
                    buyout_button:Click()
                end
