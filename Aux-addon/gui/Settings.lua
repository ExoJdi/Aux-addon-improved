...
local function clear_item_cache()
	_G.aux_items = {}
	_G.aux_item_ids = {}
...
local function build()
	...
	local scale = add_scale(content, y - 18)
	tinsert(frame._widgets, scale)

	-- Action buttons
	local clear_btn = gui.button(content, gui.font_size.small)
	gui.set_size(clear_btn, 150, 22)
	clear_btn:SetPoint('TOPLEFT', 14, y - 70)
	clear_btn:SetText('Clear item cache')
