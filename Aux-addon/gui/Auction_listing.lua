...
local TIME_LEFT_STRINGS = {
	color.red'30m', 
	color.orange'2h', 
	color.yellow(_VERSION and '12h' or '8h'), 
	color.blue(_VERSION and '48h' or '24h'), 
}
...
M.search_columns = {
    ...
    },
    {
        ...
        align = 'CENTER',
        fill = function(cell, record)
			local status = record.sale_status
			if status == 1 then
				cell.text:SetText(color.green'Sold')
			elseif status == 2 then
				cell.text:SetText(color.red'Expired')
			else
...
local methods = {
    ...
    UpdateRowInfo = function(self)
	    ...
	    local group_key_fn = self.group_key_fn or function(record) return record.item_key end
	    local first_group_key = records[1] and (group_key_fn(records[1]) or records[1].item_key)
	    local single_item = not first_group_key or all(records, function(record) return (group_key_fn(record) or record.item_key) == first_group_key end)

		sort(records, function(a, b)
			local key_a = group_key_fn(a) or a.item_key
			local key_b = group_key_fn(b) or b.item_key
			if key_a ~= key_b then
				return key_a < key_b
			end
			return a.search_signature < b.search_signature or a.search_signature == b.search_signature and tostring(a) < tostring(b)
		end)

        for i = 1, getn(records) do
            local record = records[i]
            local prevRecord = records[i - 1]
	        local record_group_key = group_key_fn(record) or record.item_key
	        local prev_group_key = prevRecord and (group_key_fn(prevRecord) or prevRecord.item_key)
	        ...
	        elseif not single_item and prevRecord and record_group_key == prev_group_key then
	            ...
	            tinsert(self.rowInfo, O('item_key', record.item_key, 'expandKey', record_group_key, 'children', A(O('count', 1, 'record', record))))
                self.rowInfo.numDisplayRows = self.rowInfo.numDisplayRows + 1
            end
        end
    ...
    UpdateRows = function(self)
        ...
        for _, cell in pairs(self.headCells) do
            ...
            tex:SetTexture[[Interface\AddOns\Aux\WorldStateFinalScore-Highlight]]
    ...
    SetDatabase = function(self, database)
        ...
        if self.handlers.OnDatabaseChanged then
            self.handlers.OnDatabaseChanged(self)
        end
    end,
