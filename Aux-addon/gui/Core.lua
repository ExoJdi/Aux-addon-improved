...
function M.item(parent)
    ...
    local blizz_border = _G[btn:GetName() .. 'Border']
    if blizz_border then
        blizz_border:SetTexture(nil)
        blizz_border:Hide()
    end
    local blizz_normal = _G[btn:GetName() .. 'NormalTexture']
    if blizz_normal then
        blizz_normal:SetTexture(nil)
        blizz_normal:Hide()
    end
    local icon_border = CreateFrame('Frame', nil, btn)
    icon_border:SetPoint('TOPLEFT', btn, 'TOPLEFT', -1, 1)
    icon_border:SetPoint('BOTTOMRIGHT', btn, 'BOTTOMRIGHT', 1, -1)
